...
class GazetteerTest(TestCase):
    ...
    def test_get_gazetteer_entry(self):
        if settings.USE_WORLDMAP:
            ...
            entry = results[0]
            self.assertEqual(5, entry["id"])
            self.assertEqual("Paradise5", entry["placename"])
    ...
    def test_gazetteer_placename(self):
        if settings.USE_WORLDMAP:
            ...
            self.assertEqual(1, len(placenames))
            self.assertEqual("Paradise2", placenames[0]["placename"])
    ...
    def test_gazetteer_placename_xml(self):
        if settings.USE_WORLDMAP:
            ...
            self.assertEqual(1, len(placenames))
            self.assertEqual("Paradise2", placenames[0].find("placename").text)
    ...
    def test_gazetteer_layer(self):
        if settings.USE_WORLDMAP:
            ...
            placenames = json.loads(response.content)
            self.assertEqual(3, len(placenames))
    ...
    def test_gazetteer_map(self):
        if settings.USE_WORLDMAP:
            ...
            placenames = json.loads(response.content)
            self.assertEqual(5, len(placenames))
            self.assertContains(response, text="base:CA1", html=False, status_code=200)
    ...
    def test_gazetteer_project(self):
        if settings.USE_WORLDMAP:
            ...
            placenames = json.loads(response.content)
            self.assertEqual(4, len(placenames))
